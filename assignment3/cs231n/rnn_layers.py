...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    pos_exp = np.exp(np.dot(x, Wx) + np.dot(prev_h, Wh) + b)
    neg_exp = 1 / pos_exp
    next_h = (pos_exp - neg_exp) / (pos_exp + neg_exp)
    # cache = (x, prev_h, Wx, Wh, b, pos_exp, neg_exp)
    cache = (x, prev_h, Wx, Wh, b, next_h)      # 上面的是朴素的求梯度，实际
    # 如果注意到dtanh(x) / dx = 1 - tanh(x)^2，则cache的时候可以用next_h代替pos_exp
    # 和neg_exp
...
def rnn_step_backward(dnext_h, cache):
    ...
    # x, prev_h, Wx, Wh, b, pos_exp, neg_exp = cache
    # dtmp = 4 * dnext_h / ((pos_exp + neg_exp) ** 2)   # tmp代表tanh激活之前的那个变量
    x, prev_h, Wx, Wh, b, next_h = cache    # 相对简洁一点的实现
    dtmp = dnext_h * (1 - next_h ** 2)      # 主要方便后面把rnn层堆起来的时候存cache
    dx = np.dot(dtmp, Wx.T)
    dWx = np.dot(x.T, dtmp)
    dprev_h = np.dot(dtmp, Wh.T)
    dWh = np.dot(prev_h.T, dtmp)
    db = np.sum(dtmp, axis=0, keepdims=False)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        # print(h[:, t, :].shape)
        h[:, t, :] = h_t
        # cache.append((x[:, t, :], prev_h, Wx, Wh, b, h_t))
        prev_h = h_t
...
def rnn_backward(dh, cache):
    ...
    #     dWh += dWh_t
    #     db += db_t
    #     dnext_h = dh[:, t-1, :] + dprev_h_t
    #     if t == 0:
    #         dh0 = dprev_h_t
        
    

    # 另一种实现(也可以，就是不如上面先构造出各个梯度的形状 那样直观)
    N, T, H = dh.shape
    dnext_h = dh[:, -1, :]
    dx, dWx, dWh, db = [], [], [], []
    for t in reversed(range(T)):
        ...
        if t == 0:
            dh0 = dprev_h_t
    dx = np.stack(list(reversed(dx)), axis=1)   # 这里很容易错，注意时间顺序
    # 所以要逆序一下。以及reversed()返回的是一个迭代器，所以还需要把它转换成列表
    dWx = np.sum(dWx, axis=0)   # 注意要指定axis=0，否则就是把所有的元素进行求和了
...
def sigmoid(x):
    ...
    # 之所以没有直接按照sigmoid的定义算，而是绕了一下，就是因为，如果x里有绝对值很大的
    # 负数，那么1/(1+e^(-x))中，分母就会出现一个很大的正数出现在e的指数位置上，有可能
    # 导致数值不稳定
    pos_mask = x >= 0
    neg_mask = x < 0
    z = np.zeros_like(x)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    N, H = prev_h.shape
    _, D = x.shape
    activation_vector = np.dot(x, Wx) + np.dot(prev_h, Wh) + b  # (N, 4H)
    i = sigmoid(activation_vector[:, :H])
    f = sigmoid(activation_vector[:, H:2*H])
    o = sigmoid(activation_vector[:, 2*H:3*H])
    g = np.tanh(activation_vector[:, 3*H:4*H])
    next_c = f * prev_c + i * g
    next_h = o * np.tanh(next_c)    # 这里的o其实基本就是朴素rnn中的结果了，但lstm多
    # 一个类似于控制项的tanh(next_c)，而next_c的计算又引入了所谓“遗忘门”，“输入门”机制
    cache = (next_h, next_c, i, f, o, g, Wx, Wh, prev_h, prev_c, x)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    next_h, next_c, i, f, o, g, Wx, Wh, prev_h, prev_c, x = cache
    N, H = dnext_h.shape
    D, _ = Wx.shape
    # do是很容易算的，然后要注意的是，dnext_c需要重新计算一下
    # 因为传进来的参数是外面计算得到的，外面利用next_h和next_c进行计算的时候，
    # 是把二者当成独立变量来看，然后求导得到的梯度；而实际上，next_c是由next_h
    # 计算得到的，因此这里还有一部分梯度需要求
    do = dnext_h * tanh(next_c)
    dnext_c = dnext_c + dnext_h * o * (1 - tanh(next_c) ** 2)
    dprev_c = dnext_c * f
    
    # 求出正确的dnext_c后，又有dnext_h，剩下的就很容易了
    di = dnext_c * g
    dg = dnext_c * i
    df = dnext_c * prev_c
    dactivation_vector = np.zeros((N, 4*H))
    dactivation_vector[:, :H] = di * i * (1-i)
    dactivation_vector[:, H:2*H] = df * f * (1-f)
    dactivation_vector[:, 2*H:3*H] = do * o * (1-o)
    dactivation_vector[:, 3*H:4*H] = dg * (1 - g ** 2)
    dWx = np.dot(x.T, dactivation_vector)
    dWh = prev_h.T @ dactivation_vector
    db = np.sum(dactivation_vector, axis=0)
    dx = dactivation_vector @ Wx.T
    dprev_h = dactivation_vector @ Wh.T
...
def lstm_backward(dh, cache):
    ...
    dflow_next_h = np.zeros((N, H))     # 这个梯度意思是，它只是时间步向前迭代时产生的对h的梯度
    # 而对每个时间步的h的梯度，其实有两部分，一部分是这个dflow_next_h，另一部分是用h进行其它计算
    # 产生的梯度，也就是dh[:, t, :]
    # 这里只考虑这个，方便后面统一代码
    # （另外，关于这里为什么初始化为0，是因为最后一个隐状态之后，就没有时间步了，对它来说，
    # 由时间步向前迭代产生的梯度是不存在的，或者说是为0的，只有其它计算产生的，也就是
    # dh[:, -1, :]）
    for t in reversed(range(T)):
        dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dflow_next_h + dh[:, t, :], dnext_c, cache[t])
        dx[:, t, :] = dx_t
        dWh += dWh_t
        dWx += dWx_t
        db += db_t
        dnext_c = dprev_c
        dflow_next_h = dprev_h
    dh0 = dflow_next_h
