...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.sum(z_i * z_j) / (torch.linalg.norm(z_i, 2) * torch.linalg.norm(z_j, 2))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denorm2 = 0
        for j in range(2 * N):
            denorm += torch.exp(sim(z_k, out[j]) / tau)
            denorm2 += torch.exp(sim(z_k_N, out[j]) / tau)
        denorm -= torch.exp(sim(z_k, z_k) / tau)
        denorm2 -= torch.exp(sim(z_k_N, z_k_N) / tau)
        l_k_kN = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denorm)
        l_kN_k = -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / denorm2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # print(torch.linalg.norm(out_left, 2, 1).shape)
    # print(out_left.shape)
    normalized_left = out_left / torch.linalg.norm(out_left, 2, 1, keepdim=True)
    normalized_right = out_right / torch.linalg.norm(out_right, 2, 1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    normalized_out = out / torch.linalg.norm(out, 2, 1, keepdim=True)
    sim_matrix = torch.matmul(normalized_out, normalized_out.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.exp(torch.cat((sim_pos_pairs, sim_pos_pairs), dim=0) / tau)
