...
def discriminator_loss(logits_real, logits_fake):
    ...
    # print('real logits shape:', logits_real.shape)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        # Unflatten(batch_size, 1, 28, 28),     # 这个其实可有可无，因为目前dataloader
        # 里的数据就已经是三维的形状了（不考虑batch size时），因此相当于已经unflatten了
        # 还是要明确输入数据的形状
        nn.Conv2d(1, 32, 5, 1),
        nn.LeakyReLU(0.01),
        nn.MaxPool2d(2, 2),
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            if (iter_count % show_every == 0):
                ...
                images.append(imgs_numpy[0:16]) # 保存一下生成出来的图像
